...
int choice;
float m;
void *p;              /*指针的使用。在主函数中通过将指针p分别指向两个求最大值和最小值的自定义函数，实现这两个函数的调用*/
float *q=a;          //用指针q指向数组a，实现对数组元素的操作


void welcome()               //该函数用于输出用户欢迎界面 
{
	cout<<"                        -------------------------------------\n";
	cout<<"                        *                                   *\n";
	cout<<"                               欢迎进入数据管理系统          \n";
...
void cut_in()                             // 选择操作1:  用于根据用户需求插入数据并输出在屏幕上
{       
        int n;
...
float sum_(int x)                                       /*选择操作2:  递归函数的使用。通过使用递归函数计算数组a所有元素的和。*/
{
	if(x==0) return a[0];
	x--;
	return sum_(x)+a[x+1];
...
float average()                               /*选择操作3:  函数嵌套的使用。在该函数中嵌套调用了sum_(int x)函数，用于计算数组a中所有元素的平均值*/           
{
	return sum_(len)/len;
}
...
void max()                                     // 选择操作4:  用于找出数组a中最大的数的位置                             
{                                                          //并支持显示几个相同的最大数位置
	   int count_m=0;
	   int count_max=0;
	   cout<<"最大的数是第";
...
void min()                                     // 选择操作4:  用于找出数组a中最小的数的位置                          
{                                                          //并支持显示几个相同的最小数位置
       int count_m=0;
	   int count_min=0;
	   m=q[0];
...
void exit()                           // 选择操作5:  根据键盘输入的数，查找是否存在                            
{                                     
 ...
 //该函数带有默认参数 e                                               
void count_choice(int e=0)             /* 选择操作6:  根据用户的选择进行报数操作并输出剩下的数和它们的下标*/
{
      int d;
      bool back;                       //用于执行用户选择超界时的返回重新操作
	  while(back)
