...
#define g_Encrypt_Path "/home/encrypt/origin/"
#define g_After_Encrypt_Path "/home/encrypt/after/"
...
void GetFile()
{
  DIR *dirptr;
  struct dirent *entry;
  if( (dirptr = opendir(g_Encrypt_Path)) == NULL) 
  {
  	char temp[100];
  ...
  while(entry = readdir(dirptr))
	{	
	    ...
	    {
	    	char AllPath[256];
	    	strcpy(AllPath,g_Encrypt_Path);
	    	strcat(AllPath,entry->d_name);
	    	printf("检查到新的txt文件,文件名:%s.\n",entry->d_name);
	    	
	    	//加密
	    	if(Encrypt(entry->d_name))
				{
...
{
	char oldpath[256];
	strcpy(oldpath,g_Encrypt_Path);
	strcat(oldpath,filename);
	
	char newpath[256];
	strcpy(newpath,g_After_Encrypt_Path);
